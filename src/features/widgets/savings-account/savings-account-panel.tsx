...
export class SavingsAccountPanel extends React.Component<
  SavingsAccountPanelProps,
  SavingsAccountPanelState
> {
  ...
  renderPopupContent = () => {
    let platforms;
    let items = this.props.component?.supportedDevices?.filter(
      (el) => el?.supportedPlatforms?.length > 0
    );
        ...
        <div
          className={
            index !== items.length - 1
              ? "flex pt2 bo0 b--solid bob1 primary-s3-bb "
              : "flex pt2"
          }
        >
      ...
      <PanelContent>
        <PanelControl>
          <TextInput
            ...
            onBlur={this.changeComponentName}
          />
        </PanelControl>
        <div className="flex flex-row">
          <div className="flex-row w-70">
            <PanelControl title="Device Compatibility"></PanelControl>
          </div>
          <PopoverWidget
            className=""
        ...
        <PanelControl title="Font">
          <DropDown
            value={this.props.component.style.font || ""}
  ...
  render() {
    return <Panel>{this.renderDesignPanel()}</Panel>;
  }
}
